...
class admittance_controller:
    ...
    class ParamListener:
        ...
        def refresh_dynamic_parameters(self):
            ...
            for value_1 in updated_params.joints:
                for value_2 in updated_params.dof_names:
                    for value_3 in updated_params.joints:
                        for value_4 in updated_params.dof_names:
            ...
            for value_1 in updated_params.dof_names:
            ...
            for value_1 in updated_params.nested_map.entries:

                updated_params.nested_map.add_entry(value_1)
                entry = updated_params.nested_map.get_entry(value_1)
                param_name = f"{self.prefix_}nested_map.{value_1}.value"
                if not self.node_.has_parameter(self.prefix_ + param_name):
                    descriptor = ParameterDescriptor(description="A value in the nested map with sibling keys", read_only = False)
        ...
        def update(self, parameters):
            ...
            for param in parameters:
                ...
                if param.name == self.prefix_ + "nested_map.entries":
                    updated_params.nested_map.entries = param.value
                    self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
                ...
                if param.name == self.prefix_ + "admittance.acceleration_limits":
                    validation_result = ParameterValidators.fixed_size(param, 3)
                    if validation_result:
                        return SetParametersResult(successful=False, reason=validation_result)
                    validation_result = ParameterValidators.lower_element_bounds(param, -10)
                    if validation_result:
                        return SetParametersResult(successful=False, reason=validation_result)
                    validation_result = ParameterValidators.upper_element_bounds(param, 10)
                    if validation_result:
                        return SetParametersResult(successful=False, reason=validation_result)
                    updated_params.admittance.acceleration_limits = param.value
                    self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
            ...
            for param in parameters:
                    ...
                    for value_1 in updated_params.nested_map.entries:
                        ...
                        if param.name == param_name:
                            ...
                            updated_params.nested_map.get_entry(value_1).value = param.value
                            self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
        ...
        def declare_params(self):
            ...
            if not self.node_.has_parameter(self.prefix_ + "dof_names"):
                descriptor = ParameterDescriptor(description="specifies which joints will be used by the controller", read_only = False)
                descriptor.additional_constraints = "Any string can be here. For example, you might want to embed JSON schema"
            ...
            if not self.node_.has_parameter(self.prefix_ + "nested_map.entries"):
                descriptor = ParameterDescriptor(description="Keys for nested mapped parameters", read_only = False)
                parameter = updated_params.nested_map.entries
                self.node_.declare_parameter(self.prefix_ + "nested_map.entries", parameter, descriptor)
            ...
            if not self.node_.has_parameter(self.prefix_ + "admittance.acceleration_limits"):
                descriptor = ParameterDescriptor(description="specifies maximum acceleration limits for x, y and z axis", read_only = False)
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = -10
                descriptor.floating_point_range[-1].to_value = float('inf')
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = -float('inf')
                descriptor.floating_point_range[-1].to_value = 10
                parameter = rclpy.Parameter.Type.DOUBLE_ARRAY
                self.node_.declare_parameter(self.prefix_ + "admittance.acceleration_limits", parameter, descriptor)
            ...
            self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
            updated_params.pid.rate = param.value
            param = self.node_.get_parameter(self.prefix_ + "nested_map.entries")
            self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
            updated_params.nested_map.entries = param.value
            ...
            updated_params.admittance.stiffness = param.value
            param = self.node_.get_parameter(self.prefix_ + "admittance.acceleration_limits")
            self.logger_.debug(param.name + ": " + param.type_.name + " = " + str(param.value))
            validation_result = ParameterValidators.fixed_size(param, 3)
            if validation_result:
                raise InvalidParameterValueException('admittance.acceleration_limits',param.value, 'Invalid value set during initialization for parameter admittance.acceleration_limits: ' + validation_result)
            validation_result = ParameterValidators.lower_element_bounds(param, -10)
            if validation_result:
                raise InvalidParameterValueException('admittance.acceleration_limits',param.value, 'Invalid value set during initialization for parameter admittance.acceleration_limits: ' + validation_result)
            validation_result = ParameterValidators.upper_element_bounds(param, 10)
